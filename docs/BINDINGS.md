...
| `:MDView selection [action]` | optional `on`\|`off`\|`toggle`, completed | Switches whether the Neovim visual selection (`v`/`V`/`CTRL-V`) is mirrored into the preview as a live highlight (`browser.selection_sync`, default `on` — it is meant for presenting, where nobody wants to have switched it on first). Sets the config and, while a session runs, pushes a live `/control` update (`{ selection = ... }`); switching it off clears a highlight that is currently drawn. No argument toggles. |
...
| `ModeChanged` (into/out of `v`/`V`/`CTRL-V`), `CursorMoved` | [`bindings/autocmds/selection_sync.lua`](../lua/mdview/bindings/autocmds/selection_sync.lua) | Sends the current visual selection (mode, lines, byte columns) to the relay, throttled with a trailing update, so the browser preview highlights the same text — and clears it on leaving visual mode. Gated per event behind `browser.selection_sync` (default `true`), so `:MDView selection on` takes effect on the running session. |
