...
| `browser.preserve_blank_lines` | `false` | Render runs of ≥2 consecutive blank lines as visible vertical space instead of collapsing them (CommonMark default). Sourcepos-safe, so scroll sync and the cursor caret stay accurate; blank lines inside fenced code are always preserved. |
