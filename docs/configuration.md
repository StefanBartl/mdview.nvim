...
| `transport.health_poll_ms` / `transport.health_timeout_ms` | `200` / `15000` | How often, and for how long in total, the relay is polled while waiting for it to come up. Raise both together on a slow box — a first-run binary being scanned by antivirus, a busy CI machine. |
