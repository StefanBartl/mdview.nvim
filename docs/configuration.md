...
| `browser.selection_sync` | `true` | Mirror the Neovim visual selection (`v`/`V`/`CTRL-V`) into the preview as a live highlight — for showing a document to other people, where pointing in Neovim is invisible to whoever is watching the browser. On by default; switch it off with `:MDView selection off`. |
