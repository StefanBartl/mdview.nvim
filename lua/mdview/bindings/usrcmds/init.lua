...
---
--- All four are registered once at setup() and never torn down — user
--- commands are the plugin's permanent command surface (like every other
--- Neovim plugin's :Commands), not something to attach/detach per preview
--- session. Only autocommands (mdview.bindings.autocmds) have a real
--- attach/detach lifecycle, since those genuinely need to stop firing once
--- a session ends.
