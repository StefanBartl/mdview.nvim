...
local function run_case(name, old_lines, new_lines)
  ...
  local ok = true
  if #patched ~= #new_lines then
    ok = false
  end
  if ok then
    for i = 1, #new_lines do
      if patched[i] ~= new_lines[i] then
        ok = false
        break
      end
    end
  ...
  for _, d in ipairs(diffs) do
    changed = changed + (d.count or 0)
  end
  local total = math.max(1, #new_lines)
  local change_ratio = changed / total

  print(
    ("[test] %s: ok=%s diffs=%d time_ms=%.3f changed=%d ratio=%.3f"):format(
      name,
      tostring(ok),
      #diffs,
      took,
...
local tests = {
  ...
  {
    name = "single_insert_middle",
    old = (function()
      local t = make_lines("X", 10)
      table.insert(t, 6, "NEW")
      return t
    end)(),
    new = make_lines("X", 11),
  },
  {
    name = "single_delete_middle",
    old = (function()
      local t = make_lines("X", 11)
      table.remove(t, 6)
      return t
    end)(),
    new = make_lines("X", 10),
  ...
  {
    name = "large_append",
    old = make_lines("P", 1000),
    new = (function()
      local t = make_lines("P", 1000)
      for i = 1001, 1500 do
        t[i] = "P" .. i
      end
      return t
  ...
  {
    name = "many_small_changes",
    old = make_lines("S", 1000),
    new = (function()
      local t = make_lines("S", 1000)
      for i = 1, 1000, 50 do
        t[i] = "Smod" .. i
      end
      return t
