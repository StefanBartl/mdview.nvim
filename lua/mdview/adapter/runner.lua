...
local function open_log_buffer()
	...
	for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
		if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_buf_get_name(bufnr) == LOG_BUF_NAME then
			-- populate and show
	...
	local bufnr = vim.api.nvim_create_buf(false, true) -- listed=false, scratch
...
function M.start_server(cmd, args, cwd)
	...
	if spawn_cmd and (spawn_cmd:match("npm") or spawn_cmd == "npm.cmd") then
		if not file_exists(path_join(spawn_cwd, "package.json")) then
			vim.schedule(function()
				notify(
					"mdview: npm start aborted — no package.json found in cwd: " .. tostring(spawn_cwd),
	...
	stdout:read_start(function(read_err, data)
		...
		if data then
			log.append(data, "[mdview]")
			if DEBUG then
				-- only show to user in debug mode, sanitized
				vim.schedule(function()
