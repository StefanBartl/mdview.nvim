			...
			if code == 0 then
				...
				if (uv.now() - start_time) < timeout then
					...
					if attempt % 10 == 0 then
						api.nvim_out_write(string.format("[mdview] waiting for server, attempt %d...\n", attempt))
					end
					vim.defer_fn(poll, HEALTH_POLL_MS)
				else
					api.nvim_err_writeln("[mdview] server health-check timed out after " .. tostring(timeout) .. "ms")
					cb(false)
				end
			end
		end)
  ...
  if curl then
    ...
    local jid = fn.jobstart(vim.list_extend({ curl }, args), {
      ...
      on_stdout = function(_, data, _)
        if data and #data > 0 then
          for _, line in ipairs(data) do
            if line and line ~= "" then table.insert(stdout_acc, line) end
          end
        end
      end,
      on_stderr = function(_, data, _)
        if data and #data > 0 then
          for _, line in ipairs(data) do
            if line and line ~= "" then table.insert(stderr_acc, line) end
          end
        end
      end,
      on_exit = function(_, code, _)
  ...
  http_post_nonblocking(url, entry.markdown, function(code, stdout_lines, stderr_lines)
    if code == 0 then
      ...
      if stdout_lines and #stdout_lines > 0 then
        ...
        api.nvim_out_write("[mdview.ws_client] http_post_nonblocking success for " .. url .. "\n")
        api.nvim_out_write("[mdview.ws_client] response body (truncated 2k):\n" .. body:sub(1, 2000) .. "\n")
      else
        api.nvim_out_write("[mdview.ws_client] http_post_nonblocking success for " .. url .. " (empty body)\n")
      end
    else
      -- failure: retry/backoff as before
      if stderr_lines and #stderr_lines > 0 then
        api.nvim_err_writeln("[mdview.ws_client] http_post stderr: " .. table.concat(stderr_lines, "\n"))
      end
...
if opts.immediate then
  http_post_nonblocking(render_url_for(path), markdown, function(code, stdout_lines, stderr_lines)
    if code == 0 then
      if stdout_lines and #stdout_lines > 0 then
        local body = table.concat(stdout_lines, "\n")
        api.nvim_out_write("[mdview.ws_client] immediate post success for " .. render_url_for(path) .. "\n")
        api.nvim_out_write("[mdview.ws_client] immediate response (truncated 2k):\n" .. body:sub(1, 2000) .. "\n")
      else
        api.nvim_out_write("[mdview.ws_client] immediate post success (empty body)\n")
      end
    else
      if stderr_lines and #stderr_lines > 0 then
        api.nvim_err_writeln("[mdview.ws_client] immediate post stderr: " .. table.concat(stderr_lines, "\n"))
      else
        api.nvim_err_writeln("[mdview.ws_client] immediate post failed for " .. path)
      end
    end
  end)
  return
