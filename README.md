...
| `browser.cursor_marker` | `"line"` | Overlay a marker in the preview at the Neovim cursor: `"line"` (a blinking bar at the cursor line), `"caret"` (a caret at the exact cursor column, via inline source-position spans from the renderer — falls back to the line marker on blank lines and inside code blocks), `"section"` (spotlight the current heading section and dim the rest — robust under video compression for a passive viewer), or `"off"`. Rides the scroll-sync ping, so needs `scroll_sync` on. Switch at runtime with `:MDViewCursor`. |
